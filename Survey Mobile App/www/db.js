...
function createTable(){
    db.transaction(function (tx) {
        ...
        tx.executeSql('CREATE TABLE IF NOT EXISTS Survey_Master (id INTEGER PRIMARY KEY, Surveyid, Name, Startdate, Enddate, Completedstatus, SyncStatus)');
        tx.executeSql('CREATE TABLE IF NOT EXISTS Category_Master (id INTEGER PRIMARY KEY, Surveyid, Categoryid, Categoryname)');
        tx.executeSql('CREATE TABLE IF NOT EXISTS Question_Master(id INTEGER PRIMARY KEY, Surveyid, Categoryid, QuestionId, Type, Question, Answer, Remarks, Importance, Answerdate, NotApplicable)');
    });
}
  ...
  db.transaction(function(tx){
        tx.executeSql('SELECT * FROM Survey_Master',[] ,function (tx, results) {
            ...
            for(var i=0;i<firstLength;i++) {
                ...
                existingSurveyExpiryDate.push(results.rows.item(i).Enddate);                                        
            }

            for (var key in data.Survey) {

              if(jQuery.inArray(data.Survey[key].surveyId, existingSurveyId) != -1) {
                data.Survey[key] = '';
              }
            }

            tx.executeSql('SELECT * FROM Category_Master',[] ,function (tx, results) {
              var existingCategoryId = [];
              categoryLength = results.rows.length, categoryLength;

              for(var i=0;i<categoryLength;i++) {
                existingCategoryId.push(results.rows.item(i).Categoryid);                                      
              }

              for (var key in data.Category) {
                if(jQuery.inArray(data.Category[key].categoryId, existingCategoryId) != -1) {
                  data.Category[key] = '';
                }
              }
            });
            ...
            for (var key in data.Question) {
              if(jQuery.inArray(data.Question[key].surveyid, existingSurveyId) != -1) {
                data.Question[key] = '';
              }
            }
    ...
    setTimeout(function() {
      db.transaction(function(tx){
        for(var i=0; i<data.Survey.length; ++i){
          if(data.Survey[i] != '') {
              tx.executeSql('INSERT INTO Survey_Master(Surveyid,Name,Startdate,Enddate,Completedstatus,SyncStatus) VALUES (?,?,?,?,?,?)',[data.Survey[i].surveyId,data.Survey[i].surveyName,data.Survey[i].surveyStart,data.Survey[i].surveyEnd,'N','N']); 
          }            
...
function insertIntoCategoryMaster() {
    db.transaction(function(tx){
        for(var i=0;i<data.Category.length;++i){  
          if(data.Category[i] != '') {
                tx.executeSql('INSERT INTO Category_Master(Surveyid,Categoryid,Categoryname) VALUES (?,?,?)',[data.Category[i].surveyid,data.Category[i].categoryId,data.Category[i].categoryName]);
          }
        }
    });
...
function insertIntoQuestionMaster(){
    db.transaction(function(tx){
        for(var i=0;i<data.Question.length;++i){
          if(data.Question[i] != '') {
                tx.executeSql('INSERT INTO Question_Master(Surveyid,Categoryid,QuestionId,Type,Question,Importance) VALUES (?,?,?,?,?,?)',[data.Question[i].surveyid,data.Question[i].categoryid,data.Question[i].questionId,data.Question[i].type,data.Question[i].question,'High']);
          }
        }
    });
}
...
function updateAnswerOnServer(userid){
      ...
      var uploadId=[];
     db.transaction(function(tx){
        tx.executeSql('SELECT * FROM Survey_Master WHERE Completedstatus = ? AND SyncStatus = ?', ['Y', 'N'], function (tx, results) {
            firstLength = results.rows.length, firstLength;
            for(var i=0;i<firstLength;i++){
     ...
     window.setTimeout(function() {     
      db.transaction(function(tx){
        for(var j=0;j<uploadId.length;j++){
            tx.executeSql('SELECT * FROM Question_Master WHERE Surveyid = ?', [uploadId[j]], function (tx, results) {
            ...
            for(var i=0;i<secondLength;i++){          
                  var answer={
                        ...
                        "Date":results.rows.item(i).Answerdate,
                        "DataFrom":"MOBILE"
                  };
                submitArray.push(answer);
            }     
            //localStorage.setItem('answerObject', JSON.stringify(submitArray));
            },null);
        }         
    });
        window.setTimeout(function() {  
...
function generateJson(){
    ...
    db.transaction(function(tx){
        tx.executeSql('select DISTINCT QuestionId, Surveyid, Categoryid, Type, Question, Answer, Remarks, Importance, NotApplicable FROM Question_Master QuestionId',[],function(tx,results){                    
    ...
    window.setTimeout(function() {
      /*  console.log("firstObject "+firstObject.rows.item(0).Name +"  "+firstLength);
        console.log("secondObject "+secondObject.rows.item(0).Categoryname+"  "+secondLength);
        console.log("thirdObject "+thirdObject.rows.item(0).Question+"  "+thirdLength);*/

        for(var i=0;i<firstLength;i++){
            var dataobjects={};
            var first=firstObject.rows.item(i).Surveyid;
            dataobjects["surveyId"]=firstObject.rows.item(i).Surveyid;
            dataobjects["surveyName"]=firstObject.rows.item(i).Name;
            dataobjects["surveyStart"]=firstObject.rows.item(i).Startdate;
            dataobjects["surveyEnd"]=firstObject.rows.item(i).Enddate;
            dataobjects["syncStatus"]=firstObject.rows.item(i).SyncStatus;
            ...
            for(var j=0;j<secondLength;j++){
                ...
                if(!(secondObject.rows.item(j).Categoryid in categoryCompleteList)) {
                  var category={
                                 "categoryId": secondObject.rows.item(j).Categoryid,
                                 "categoryName": secondObject.rows.item(j).Categoryname
                            };
            ...
            for(var k=0;k<thirdLength;k++){  
                if(first==thirdObject.rows.item(k).Surveyid){              
                        var Question={
                            ...
                            "notApplicable": thirdObject.rows.item(k).NotApplicable
                            };
                            ...
                            if(!(thirdObject.rows.item(k).Categoryid in categoryList)) {
                              ...
                              for (var property in dataobjects.categoryCompleteList) {
                                  if((dataobjects.categoryCompleteList).hasOwnProperty(property) ) {
                                      if(dataobjects.categoryCompleteList[property].categoryId == thirdObject.rows.item(k).Categoryid) {
...
function updateAnswers(updateData, completed){
    ...
    db.transaction(function(tx) {
        for(var i=0;i<updateData.length;i++){
          var answerArray = (updateData[i].answer).split('&;'),
          ...
          var surveyId = parseInt(updateData[i].surveyId, 10);
          var questionId = parseInt(updateData[i].questionId, 10);

            tx.executeSql('UPDATE Question_Master SET Answer = ? , Remarks = ?, Answerdate = ?, NotApplicable = ? WHERE Surveyid = ? AND QuestionId= ?',[answer, remarks, updateData[i].answerDateTime, updateData[i].notApplicable ,surveyId, questionId]);
            
            if(completed) {
              tx.executeSql('UPDATE Survey_Master SET Completedstatus = ? WHERE Surveyid = ?',['Y', surveyId]); 
            }
        }
...
function updateSurveySyncStatus(surveyId) {
  var sId = parseInt(surveyId, 10);
  db.transaction(function(tx){
    tx.executeSql('UPDATE Survey_Master SET SyncStatus = ? WHERE Surveyid = ?',['Y', sId]);
  });
}
...
function insertAllocatedData(){
    db.transaction(function(tx){
        for(var i = 0; i < allocatedData.AllocationList.length; ++i){

            tx.executeSql('INSERT INTO Userdata (LeadType, Product, Channel, FinanaceType, FinanaceCategory, FName, LName, MobileNo,Address1, Address2, Address3, City, Meetinglocation,MeetingTime,CreationDate,LeadId,Segment,AssetValue,LoanAmount,Tenure,IRR,Sync_Status,OpportunityId,Favourite,DateTime, MobilePK, UserId) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[allocatedData.AllocationList[i].Lead.LeadType, allocatedData.AllocationList[i].Lead.Product,allocatedData.AllocationList
